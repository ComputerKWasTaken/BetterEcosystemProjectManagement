...
| `core.md` | `core.js`, `ops-dispatcher.js`, and `module-registry.js`. State-channel dispatch (card-family routing by `frontier:state:*` prefix), ops dispatch (`frontier:out` consumption, dedup, idempotency, GC), adventure scoping, heartbeat coalescing and echo suppression inside `core.js`, protocol-version negotiation, debug mode. |
