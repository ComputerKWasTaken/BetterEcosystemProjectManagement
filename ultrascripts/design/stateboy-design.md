# Stateboy Product and Technical Design

## Purpose

This document records the product decisions, technical behavior, and design
options for `stateboy.js`. The current implementation is unpublished. Active
release scope and remaining milestones live in
[`planning/stateboy.md`](../planning/stateboy.md).

Stateboy is the current feature project after BetterDungeon v2.1 shipped.
...
- AI can set existing states but cannot add or remove states.
- Prompt construction uses fixed content limits instead of budgeting against
  the executor limit advertised by `ai.status`.
- Initial generation, cleanup jobs, commands, and undo/redo are not
  implemented; they are later candidates, not first-release blockers.

## Implemented Contract: Required-Mode Setup Warnings
...
3. **AI present but not ready**
   ...
   - When status reports `ready: false`, use its safe `message` when available
     and direct the user to configure the selected AI provider in BetterDungeon.
...
The implementation centralizes capability checks in a structured setup
assessment:
...
AI queuing, Widget publication, and player warnings consume this result.

## Planned Contract: Budgeted AI Context and Logging
...
- use up to 90% of the advertised `promptMaxChars` for the complete request;
...
Do not make a shared BetterDungeon executor limit increase part of the Stateboy
release by default. Consider one separately only if live evidence shows the
first-release prompt cannot work within the current contract.
...
The builder should allocate the remaining budget dynamically, retain whole
sections where practical, and report truncation in Debug Mode. Give compiled
Context space only after preserving the instructions and complete current
state sheet.
...
Keep enough bounded history for the AI and user to understand recent changes.
The exact caps below are design targets and should be retained only if they fit
the executor and Story Card limits:
...
Provider cost and request limits vary. Use available prompt space carefully,
but never treat the selected provider as unlimited or exceed the executor
contract.

## Planned Contract: AI Add and Remove Operations
...
## Implemented Contract: State Directives
...
## Later Candidate: Initial Generation and Cleanup Jobs
...
## Later Candidate: Batch Undo and Redo
...
## Later Candidate: Command System
...
- Keep scripts compatible with AI Dungeon's ES5-like script environment.
- Preserve the copy-and-paste model: shared helper behavior must remain
  contractually aligned when it is embedded in the Stateboy source.
- Keep script state and Story Card notes bounded.
- Keep user-visible publication and release milestones in
  [`planning/stateboy.md`](../planning/stateboy.md).

## Verification Scenarios

Use applicable scenarios for approved and implemented features. Scenarios for
generation, cleanup, commands, and undo/redo are conditional on those features
being prioritized after the first release.
