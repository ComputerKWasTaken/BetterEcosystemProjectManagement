...
Provider AI is the completed Phase 9 Ultrascripts capability after the Phase 8 Story Card / heartbeat cleanup. It is Ultrascripts's core AI bridge.

The goal is to let AI Dungeon scripts ask a provider-backed language model for sidecar reasoning without hijacking the story model or smuggling tool requests through story text. This is the AI path Ultrascripts should keep expanding.
...
- Provider setup, key status, model selection, and limits should be visible in BetterDungeon's Ultrascripts UI.
...
4. The module returns normalized terminal responses through the existing Full Ultrascripts dispatcher.
...
- Dispatcher path writes `ultrascripts:in:providerAI`.
...
- Local model integration. Ultrascripts should not ask normal users to configure Ollama, LM Studio, local OpenAI-compatible servers, or per-device inference stacks.
...
Move into Phase 10 guide and docs rewrite. Provider AI should be documented as the core Ultrascripts AI bridge for script-side sidecar reasoning, with follow-up work focused on stronger provider support, clearer configuration, safer request limits, and richer scenario-author examples.
